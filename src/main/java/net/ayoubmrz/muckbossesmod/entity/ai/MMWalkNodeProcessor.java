package net.ayoubmrz.muckbossesmod.entity.ai;
...
public class MMWalkNodeProcessor extends LandPathNodeMaker {
    ...
    public PathNode getStart() {
        ...
        if (this.canSwim() && this.entity.isSubmergedInWater()) {
            ...
            for (Block block = this.entity.getWorld().getBlockState(pos).getBlock(); block == Blocks.WATER; block = this.entity.getWorld().getBlockState(pos).getBlock()) {
                pos.setY(++y);
            }
            ...
            BlockPos.Mutable pos = new BlockPos.Mutable(MathHelper.floor(this.entity.getX()), y, MathHelper.floor(this.entity.getZ()));
//            while (y > 0 && (this.entity.getWorld().getBlockState(pos).isAir() || this.entity.getWorld().getBlockState(pos).getBlock().getCollisionShape(this.entity.getWorld().getBlockState(pos),this.entity.getWorld(), pos, ShapeContext.absent()) != VoxelShapes.empty())) {
                while (y > 0 && (this.entity.getWorld().getBlockState(pos).isAir() || this.entity.getWorld().getBlockState(pos).getCollisionShape(this.entity.getWorld(), pos, ShapeContext.absent()) != VoxelShapes.empty())) {
                pos.setY(y--);
            }
            y++;
        }
        ...
        if (this.entity.getPathfindingPenalty(this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y, z)) < 0.0F) {
...
//                PathNodeType pathnodetype = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y, z);
                PathNodeType pathnodetype = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, p.getX(), p.getY(), p.getZ());
    ...
    private PathNode getNode(int x, int y, int z, int step, double floor, Direction dir) {
        ...
        double dirFloor = (double) y - (1.0D - this.entity.getWorld().getBlockState(under).getCollisionShape(this.entity.getWorld(), under).getMax(Direction.Axis.Y));
        if (dirFloor - floor > 1.125D) {
            return null;
        }
        PathNodeType atNode = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y, z);
        ...
        if (result == null && step > 0 && atNode != PathNodeType.FENCE && atNode != PathNodeType.TRAPDOOR) {
            ...
            if (result != null && (result.type == PathNodeType.OPEN || result.type == PathNodeType.WALKABLE) && this.entity.getYaw() < 1.0F) {
                ...
                Box floorShape = this.entity.getWorld().getBlockState(pos).getCollisionShape(this.entity.getWorld(), pos).getBoundingBox();
...
//                PathNodeType down = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y, z);
                PathNodeType down = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y - 1, z);
            ...
            while (y-- > 0 && atNode == PathNodeType.OPEN) {
                ...
                atNode = this.getPathType(new PathContext(this.entity.getWorld(), this.entity), this.entity, x, y, z);
    ...
    private PathNodeType getPathType(PathContext pathContext, MobEntity living, int x, int y, int z) {
        return this.getNodeType(pathContext, x, y, z, living);
    }
}