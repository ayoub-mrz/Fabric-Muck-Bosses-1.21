            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    for (int y = y0; y != y1; y += stepy) {
                        ...
                        if (!block.canPathfindThrough(NavigationType.LAND)) return false;
                    }
                    PathNodeType below = this.nodeMaker.getNodeType(new PathContext(this.entity.getWorld(), this.entity), x, y0 - 1, z, this.entity);
                    if (below == PathNodeType.WATER || below == PathNodeType.LAVA || below == PathNodeType.OPEN) return false;
                    PathNodeType in = this.nodeMaker.getNodeType(new PathContext(this.entity.getWorld(), this.entity), x, y0, z, this.entity);
